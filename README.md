...
## UTF-8 First 운영 정책
- 텍스트 리소스/소스는 UTF-8 기준으로 관리합니다.
- 바이너리 산출물(`.exe`, `.arj`, `.mdp`, 실행 바이너리 blob)은 인코딩 변환 대상에서 제외합니다.
- 소켓 입력 파이프라인은 UTF-8 strict 모드입니다.
- 잘못된 UTF-8 시퀀스는 무시되며, 백스페이스는 1바이트가 아닌 1코드포인트 단위로 처리됩니다.
- 문자열 길이 제한은 바이트 길이(`strlen`)가 아니라 유니코드 코드포인트 길이 기준입니다.
- 플레이어 이름은 유니코드 문자열을 허용하며, 금지 문자(제어문자, `/` 등)는 차단됩니다.
- 플레이어 저장 경로는 이름 첫 글자 분기(`first_han`)가 아니라 SHA-1 2자리 샤딩(`player/<xx>/<name>`)을 사용합니다.
- CP949 플레이어 파일/입력과의 하위 호환은 제공하지 않습니다.
