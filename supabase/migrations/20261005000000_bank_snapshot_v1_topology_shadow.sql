...
begin
  ...
  for n in select value from jsonb_array_elements(p_nodes) loop
    if jsonb_typeof(n)<>'object' or not (n ? 'nodeIndex' and n ? 'parentNodeIndex' and n ? 'siblingOrdinal') or (n->>'nodeIndex') !~ '^[0-9]+$' or (n->>'siblingOrdinal') !~ '^[0-9]+$' or ((n->>'parentNodeIndex') is not null and (n->>'parentNodeIndex') !~ '^[0-9]+$') then raise exception using errcode='22023',message='BankSnapshotV1 topology nodes are invalid'; end if;
    if (n->>'nodeIndex')::integer<>i or (n->>'siblingOrdinal')::integer not between 0 and 8191 then raise exception using errcode='22023',message='BankSnapshotV1 topology preorder bounds are invalid'; end if;
    parent:=case when n->>'parentNodeIndex' is null then null else (n->>'parentNodeIndex')::integer end; sibling:=(n->>'siblingOrdinal')::integer;
    if parent is null then expected:=coalesce((select count(*) from jsonb_array_elements(p_nodes) x where (x->>'nodeIndex')::integer<i and x->>'parentNodeIndex' is null),0); ancestors:=array[]::integer[]; else if parent<0 or parent>=i or not parent=any(ancestors) then raise exception using errcode='22023',message='BankSnapshotV1 topology parent preorder bounds are invalid'; end if; expected:=children[parent+1]; children[parent+1]:=expected+1; ancestors:=ancestors[1:array_position(ancestors,parent)]; end if;
