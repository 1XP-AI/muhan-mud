...
harness="$repo_root/supabase/tests/player_snapshot_normalized_v1_projection_persistence_disposable_contract.sh"
fixture_path="$repo_root/tests/fixtures/player_snapshot_v1_tree_inventory.hex"

fail() {
  printf 'PlayerSnapshotNormalizedV1 disposable harness test failed: %s\n' "$1" >&2
  exit 1
}
...
umask 077
test_root="$(mktemp -d "${TMPDIR:-/tmp}/pnp-disposable-harness.XXXXXX")"
...
result="$(run_harness acknowledgement-only env -i PATH="$path_value" \
  PLAYER_SNAPSHOT_NORMALIZED_V1_PROJECTION_PERSISTENCE_ALLOW_DISPOSABLE=1 bash "$harness")"
...
rg -q --fixed-strings -- 'PlayerSnapshotNormalizedV1 projection persistence disposable contract passed' \
  "$test_root/allowed.output" || fail 'successful run must emit the aggregate success outcome'

fixture_hex="$(tr -d '\r\n' < "$fixture_path")"
expected_payload="--set=pvi_tree_payload=decode('$fixture_hex', 'hex')"
rg -Fx -- "$expected_payload" "$psql_args" >/dev/null ||
  fail 'psql must receive the exact checked-in tree-fixture SQL expression'
...
rg -Fx -- "--file=$repo_root/supabase/tests/player_snapshot_normalized_v1_projection_persistence_contract.sql" \
  "$psql_args" >/dev/null || fail 'psql must execute only the approved persistence contract'
[[ "$(rg -c --fixed-strings -- '---' "$psql_args")" == 1 ]] ||
  fail 'the harness must execute exactly one psql contract invocation'
if rg -q --fixed-strings -- 'postgresql://example.invalid/contract' "$test_root/allowed.output" \
  || rg -q --fixed-strings -- "$fixture_hex" "$test_root/allowed.output"; then
  fail 'operator output must not expose connection data or raw payload'
fi
...
rg -q --fixed-strings -- 'PlayerSnapshotNormalizedV1 projection persistence disposable contract failed' \
  "$test_root/psql-failure.output" || fail 'psql failure must emit only the aggregate failure outcome'
if rg -q --fixed-strings -- 'postgresql://example.invalid/contract' "$test_root/psql-failure.output" \
  || rg -q --fixed-strings -- "$fixture_hex" "$test_root/psql-failure.output"; then
  fail 'failed operator output must not expose connection data or raw payload'
fi

if rg -q --fixed-strings -- 'docker' "$harness" \
  || rg -q --fixed-strings -- '/supabase/migrations/' "$harness"; then
  fail 'the harness must neither create a disposable database nor apply migrations'
fi

printf 'PlayerSnapshotNormalizedV1 disposable harness hermetic tests passed\n'
