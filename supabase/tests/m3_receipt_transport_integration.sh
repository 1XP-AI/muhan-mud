...
run_super --command="update private.game_character_writer_epochs set issued_at=clock_timestamp()-interval '2 seconds', expires_at=clock_timestamp()-interval '1 second' where world_id='$world';" >/dev/null || fail "could not expire disposable A lease"
