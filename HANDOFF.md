...
사용자가 GitHub Project 기반 작업 재개와 goal 생성을 명시했다. 현재 권위는 이 블록이며, 과거 STOP은 해제되었다. live 루트는 `/Users/jjangg96/Documents/1xp/muhan-mud.nosync`, 브랜치는 `codex/mud-identity-foundation`, HEAD는 **`4e6055e`**다.
...
- 최신 정착: #7 NPC ordinary combat `MPOISS`→`PPOISN` `ctx_8a120016169c` / `task_b67992572ae2` (worker_done `msg_596526509b82`, report `/tmp/g7-npc-poison-report.md`). `src/update.c:471-490` 성공 hit 뒤 poison roll 1회·`<=15` threshold·durable summary/state·lethal probe/replay를 네 소유 파일에 반영했고, coordinator race 2회·vet·gofmt·diff-check·scope/hash 검증 후 worker release 및 #7 issue/Project Evidence 갱신을 완료했다. #7은 전체 NPC effect/producer-tick-AI/추종/리젠·운영 게이트가 남아 OPEN이며, #11 `ctx_43c752e2ffb3` / `task_94a473d75945`는 Luna max capacity 대기 중이다.
