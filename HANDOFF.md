...
- Orca Run: `run_80efe6288c63`; coordinator: `term_929886c4-466e-4f7d-a8ab-36a9cd747d15`
- goal: `01a0a045-fc22-7071-8752-614f31b7dfdb` (active)
- 커밋/보존 정책 보정: 독립 slice가 worker_done·coordinator 검증·release까지 끝나면 파일 소유 범위를 확인해 로컬 checkpoint commit 후 `private/codex/mud-identity-foundation`에 push한다. 기존 dirty 변경과 `src/frp.new`는 별도로 보호하고, 아래의 이전 `commit/push` 금지 문구는 현재 정책이 아니다. Astra는 비용 정책상 사용하지 않는다.
- 현재 GitHub 연동: PR #13 (`https://github.com/1XP-Inc/muhan-mud/pull/13`)이 Project #1에 `In Progress`로 등록되어 있고, `Refs #1`, `Refs #3`–`Refs #12`로 추적한다. 이슈는 전체 인수 전까지 OPEN으로 유지하며, PR은 현재 `REVIEW_REQUIRED`이고 보고된 CI check는 없다. 머지는 필요한 승인 후에만 수행한다.
- 현재 오케스트레이션: #12 diagnostic `ctx_eeb48d78446b` / `task_734330383ef2`는 보고서 검증·release 완료했다. #12 실제 Go terminal-only disposable browser lane `ctx_b6f398bc2185` / `task_a332fb7991c8`가 `gpt-5.6-luna` / `max`로 실행 중이며, worker는 commit/push/GitHub 변경을 하지 않고 coordinator가 검증 후 commit/push/PR 갱신을 맡는다.
- 최신 #11 정착: actual rooms/NPC/item full-data dry-run `ctx_1c5d3f67da39` / `task_94a473d75945` (worker_done `msg_8f9810addb4d`, report `/tmp/g4-full-data-actual-npc-item-report.md`)를 두 번 실행해 예상 missing-input exit 1과 byte-identical stdout SHA `96c8ecea...`를 확인했다. canonical rooms 2,341개·NPC 639개·item node 8,622개, DB/listener/output false를 고정했고 worker race 2회·coordinator race·vet·JSON/status/hash/diff-check PASS 후 release 및 #11 issue/Project Evidence 갱신을 완료했다. 여섯 full-data 입력군과 manifests/data·fencing/recovery·PG backup/PITR/cutover가 남아 OPEN이다.
- 최신 정착: #7 NPC ordinary combat `MPOISS`→`PPOISN` `ctx_8a120016169c` / `task_b67992572ae2` (worker_done `msg_596526509b82`, report `/tmp/g7-npc-poison-report.md`). `src/update.c:471-490` 성공 hit 뒤 poison roll 1회·`<=15` threshold·durable summary/state·lethal probe/replay를 네 소유 파일에 반영했고, coordinator race 2회·vet·gofmt·diff-check·scope/hash 검증 후 worker release 및 #7 issue/Project Evidence 갱신을 완료했다. #7은 전체 NPC effect/producer-tick-AI/추종/리젠·운영 게이트가 남아 OPEN이다.
- 최신 정착: #10 `command5.c:who` 중앙 라우팅 `ctx_e584eec17a21` / `task_57f83a285f8a` (report `/tmp/g10-who-routing-task_57f83a285f8a.md`, worker_done `msg_8437e1c3bedd`)는 exact `누구`·`누구 l`을 `CommandSocial`로 연결하고 malformed L/x/extra/control·그룹/무리 l은 fail-closed로 유지했다. 기존 social handler의 long `종족` projection·read-only receipt/replay 회귀를 확인했으며 coordinator race 2회·vet·gofmt·diff-check·scope/hash PASS 후 worker release 완료했다. #10은 Project `In Progress`·이슈 `OPEN` 유지, chat/board/mail/group/family/admin·full ledger/PG/E2E/browser/mobile/deploy가 남아 있다.
- 최신 정착: #3 `ctx_f88c468807fc` / `task_e1c88e7ec765`는 C quoted/multi-token target 계약 부재를 확인해 fail-closed 회귀 테스트만 추가했고, #12 `ctx_e83421019e9c` / `task_126f50d9380b`는 `cmd/muhan`·`cmd/muhan-browser-e2e` concrete Linux/ARM64 ELF build를 확인했다. 두 worker 모두 coordinator 검증·release 완료, #3/#7/#12는 Project `In Progress`·이슈 `OPEN`, active/reclaimable worker 0개다.
- 정책: 서브에이전트는 `gpt-5.6-luna` / `max`만 사용하며 Astra/Terra 등으로 자동 승격하지 않는다. 현재 active worker는 `ctx_b6f398bc2185` 1개다. #11 full-data dry-run `ctx_893d7c03076a` / `task_5ecd6d1e7e5c` (report `/tmp/g4-full-data-dry-run.md`)는 worker_done·coordinator 검증·release 완료했고, #9 study `ctx_8a1cabe509cf`, #9 training/stat `ctx_d9fb99d741a5`, #10 settings `ctx_2d70877ca549`도 coordinator 검증·release 완료.
- 이번에 정착한 worker: #3 검증 `ctx_1a2bd90c4d62`, #4 회귀 테스트 `ctx_a93f5e83db73`; 둘 다 ack/release 완료.
- #4 추가 파일: `server/internal/transport/world_connector_identity_test.go`; issue #4와 Project Evidence는 부분 증거로 갱신했지만 OPEN 유지.
- 이번 재개에서 정착한 Luna max slice: #3 follow/lose occurrence·`나` alias `ctx_1011bd560abb` / `task_208726132104` (report `/tmp/g1-follow-occurrence.md`, 3개 소유 파일, ack/release 완료). #3 issue comment와 Project Evidence를 갱신했고 OPEN 유지.
- 이번 재개에서 정착한 Luna max slice: #6 `buy_states` deterministic apply `ctx_8e6aba74d1b8` / `task_831b2f6534d0` (report `/tmp/g3-buy-states-apply.md`, 4개 소유 파일, ack/release 완료). #6 issue comment와 Project Evidence를 갱신했고 OPEN 유지.
- 이번 재개에서 정착한 Luna max slice: #7 C `display_rom` 전투 알림 parity `ctx_5a84d3b6cb75` / `task_9557eba1bd0d` (report `/tmp/g4-npc-display-combat.md`, `current_scene.go`와 테스트 2개, ack/release 완료). `Damage < 0`은 표시 경로에서만 허용하고 nil Enemies·미해결 identity·legacy room monsters 및 실제 combat/tick guard는 유지했다. positive display/MFOLLO race 2회·vet·gofmt·diff-check PASS; 두 nil fixture baseline red는 기록했고 #7 OPEN 유지.
- #9 정착한 Luna max slices: reducer `ctx_54c92fc70cc2` / `task_78b6413b8708` (report `/tmp/g3-recall-target.md`), destination-arrival follow-up `ctx_df5ceb4901eb` / `task_059cfc77ea2c` (report `/tmp/g3-recall-target-arrival.md`), targeted positive occurrence `ctx_701c6210a39c` / `task_1309c925aad9` (report `/tmp/g3-recall-occurrence.md`) 모두 worker_done·ack/release 또는 release 완료. 대상 귀환 parser/atomic transfer/C messages/source·target·destination fan-out/PHIDDN·PDMINV/replay와 `find_crt` 1-based occurrence를 Project Evidence에 반영했지만 전체 기술·퀘스트·PG/E2E·ARM64/browser/deploy acceptance가 남아 #9 OPEN 유지.
- 이번 정착 slice들의 focused race/vet/diff 검증은 PASS. 남은 #3 full exits/traps·C corpus·connected E2E, #6 legacy inventory·live transport RNG wiring·full G3, #7 NPC tick/AI/공격·추종·리젠 전체는 다음 별도 slice로 남겼다. #7의 두 nil-Enemies fixture 실패는 표시 경로를 넓히지 않고 baseline으로 보존했다.
- 기존 dirty 파일과 **`src/frp.new`**는 보존한다. 독립 검증이 끝난 slice는 checkpoint commit/push하고, issue close/Helm/deploy는 별도 승인 없이 수행하지 않는다.
- 과거 stuck G0 `task_7a36af246be8`와 replay된 heartbeat delivery는 위조·변경하지 않는다. GitHub OPEN은 #1, #3–#12, CLOSED는 #2 only.
- 2026-09-15 정착한 Luna max slice: #8 상점 C `EQUAL`/`find_obj` display-name·key[0..2] prefix selector `ctx_7a3dce18448b` / `task_f670f9284bd3` (report `/tmp/g3-shop-prefix-key.md`). canonical shop buy·pawn-sale quote/mutation의 Inventory 순서·positive occurrence·OINVIS/PDINVI·server identity/replay를 검증했고 지정된 9개 shop world/session/transport-test 파일만 변경했다. worker와 coordinator의 `Shop|Marketplace|Buy|Sell` race 2회·vet·gofmt·diff-check PASS 후 release 완료. #8 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #10 `command8.c:moon_set` C `EQUAL`/`find_obj` display-name·key[0..2] prefix selector `ctx_c19cfca2eb1d` / `task_cabef61e4304` (report `/tmp/g10-moon-set-equal.md`). direct Inventory 순서·positive occurrence·OINVIS/PDINVI·nested/Ready 제외와 기존 atomic mutation/receipt/replay를 검증했고 `moon_set.go`와 테스트 3개만 변경했다. worker와 coordinator의 `MoonSet|ParseCommandClassifiesImplemented|ParseCommandDoesNotTreatExtraTokens` race 2회·vet·gofmt·diff-check PASS 후 release 완료. #10 OPEN 유지.
- 위 두 slice의 issue comment는 REST로 기록했다(`#8` comment-5674003556, `#10` comment-5674003597). 같은 시점 Project v2 Evidence mutation은 GitHub GraphQL rate-limit 오류로 실패했으므로 Project Evidence는 API 정상화 시 동일 근거로 갱신해야 한다. 이슈 close·commit/push/deploy는 하지 않았다.
- 2026-09-15 정착한 Luna max slice: #8 `command7.c:value`·`command8.c:repair` C `EQUAL`/`find_obj` display-name·key[0..2] prefix selector `ctx_8228b9f9cd38` / `task_477997272cdf` (report `/tmp/g3-value-repair-equal.md`). canonical direct Inventory 순서·positive occurrence·중복 필드의 root 1회 계산·OINVIS/PDINVI·value nested-parent/repair parent-subtree 정책과 기존 PHIDDN·RNG·cost·mutation·replay를 검증했고 지정 8개 world/session/transport 파일만 변경했다. coordinator anchored race 2회·vet·gofmt·diff-check PASS, `src/frp.new` SHA-256 보존. worker succeeded/settled; release는 동일 request ID `6d40dcf7-6342-44ee-a8ee-c838f8e09e66` 복구 중 `release_pending`으로 상태를 기록한다. #8 issue comment `comment-5674298222`를 REST로 남겼고 #8 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #8 `command10.c:trade` C `find_crt/find_obj/EQUAL` selector/parity `ctx_bd53e4152083` / `task_18cd2d7e5a4c` (report `/tmp/g3-trade-equal.md`). room NPC·canonical direct Inventory 순서·positive occurrence·name/key prefix·MINVIS/OINVIS/PDINVI·unresolved preflight와 post-selection exact catalog name/key[0], atomic consume/reward/quest/replay를 검증했고 trade.go와 world/session/transport test 4개만 변경했다. coordinator trade race 2회·vet·gofmt·diff-check PASS 후 release 완료. real PG는 `MUHAN_TRADE_TEST_DATABASE_URL` 미설정으로 주장하지 않았다. #8 issue comment `comment-5674482676`를 REST로 남겼고 #8 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #10 `command5.c:set/clear` settings parity `ctx_2d70877ca549` / `task_8f62312510e5` (report `/tmp/g3-settings-parity.md`). C parser의 생략 `도망수치` 기본값 1, 1→10, 그 외 MAX(value,2), PWIMPY 유지 경계를 수정하고 ordinary/special/family/clear/flag-list/stale/replay 테스트를 보강했다. `server/internal/world/settings.go`, `settings_test.go`, `server/internal/session/settings_command_test.go` 3개만 변경했고 worker race 2회·vet·gofmt·diff-check 및 coordinator race/scope PASS 후 release 완료. PG 설정 테스트는 환경변수 미설정으로 skip이며 issue comment `comment-5674620085`를 REST로 남겼고 #10 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #9 `magic1.c:study` C EQUAL selector/Ready fallback `ctx_8a1cabe509cf` / `task_1772927cc901` (report `/tmp/g3-study-equal.md`). direct canonical Inventory 우선, display/key prefix·OINVIS/PDINVI·positive occurrence, Ready slot 독립 fallback, Ready root/subtree 학습 제거와 alignment room transfer를 proposal location/slot로 원자 처리했고 `server/internal/world/study.go`, `study_test.go` 2개만 변경했다. worker race 2회·vet·gofmt·diff-check 및 coordinator study race/vet/gofmt/diff-check/scope PASS 후 release 완료. issue comment `comment-5674657723`를 REST로 남겼고 전체 기술·퀘스트·PG/E2E·ARM64/browser/deploy가 남아 #9 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #9 training/level-stat parity `ctx_d9fb99d741a5` / `task_5d9001b153e3` (report `/tmp/g3-training-stat-parity.md`). C `command7.c:train`의 PUPDMG source order와 `player.c:up_level` signed legacy stat 경계를 맞추고 raw -1/-128 stat 증가 및 범위 밖 fail-closed를 보강했으며 `training.go`, `training_test.go`, `level_up.go`, `level_up_test.go` 4개만 변경했다. worker race 2회·RaiseLevel race 2회·vet·gofmt·diff-check와 coordinator 동일 검증 PASS 후 release 완료. issue comment `comment-5674678721`를 REST로 남겼고 family edit/broadcast/전체 #9 인수는 남아 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #9 `magic1.c:zap` selector parity `ctx_312350c02cc4` / `task_158139d17eaf` (report `/tmp/g3-zap-selector.md`). C `magic1.c:zap`, `object.c:find_obj`, `mtype.h:EQUAL`에 맞춰 direct Inventory의 display/key[0..2] case-insensitive prefix·OINVIS/PDINVI·positive occurrence를 적용하고, direct miss 뒤 Ready fallback counter를 독립화했다. `server/internal/world/zap.go`, `zap_test.go` 2개만 변경했으며 worker·coordinator zap/parser race 2회·vet·gofmt·diff-check·scope PASS 후 release 완료. issue comment `comment-5674806143`를 REST로 남겼고 전체 주문 catalog/퀘스트/PG/E2E/ARM64/browser/deploy 인수는 남아 #9 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #9 `magic1.c:drink` selector parity `ctx_3a60788174cf` / `task_e1c03fb93250` (report `/tmp/g3-drink-selector.md`). C `EQUAL`의 display/key[0..2] case-insensitive prefix와 Inventory 실패 뒤 독립적인 Ready occurrence counter를 `selectDrinkRoot`에 반영했고 `server/internal/world/drink.go`와 기존 dirty `drink_test.go`만 변경했다. TDD red→green, coordinator exact selector race·world/session/transport drink race·vet·gofmt·diff-check 및 `src/frp.new` SHA 보존을 확인한 뒤 worker release 완료. issue comment `comment-5675031350`를 REST PATCH로 기록했고 #9는 전체 기술·퀘스트·효과/출력·PG/E2E·ARM64/browser/deploy 인수가 남아 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #8 equipment selector parity `ctx_68c6862a2533` / `task_6ac5b01084b5` (report `/tmp/g3-equipment-selector.md`). C `command3.c` wear/ready/hold의 Inventory-only 및 remove의 Ready-only 경계를 보존하면서 장비 전용 selector에 display/key[0..2] case-insensitive EQUAL prefix·positive occurrence·OINVIS/PDINVI를 적용했고 `server/internal/world/equipment_mutation.go`, `equipment_mutation_test.go`만 변경했다. worker·coordinator focused race/vet/gofmt/diff-check/scope PASS 후 release 완료, issue comment `comment-5675211450`를 REST로 기록했으며 #8 전체 인수는 남아 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #9 use selector parity `ctx_8eb67a16abce` / `task_ffc866df4b39` (report `/tmp/g3-use-selector.md`). C `command9.c:480-513`의 Inventory 우선·floor 독립 fallback과 OUSEFL/direct-root/no-Ready 경계를 보존하면서 display/key[0..2] case-insensitive EQUAL prefix·root 1회 occurrence를 적용했고 `server/internal/world/use.go`, `use_test.go`만 변경했다. worker·coordinator focused/boundary race·vet/gofmt/diff-check/scope PASS 후 release 완료, issue comment `comment-5675211488`를 REST로 기록했으며 전체 magic/quest/effect/output/운영 인수가 남아 #9 OPEN 유지.
- 2026-09-15 coordinator-only verification: 기존 구현의 #3 last-token look 경계와 #8 live item catalog Submit 회귀를 별도 race로 재실행해 PASS했다. 새 worker/코드 변경은 없고 #3 comment `comment-5675223848`, #8 comment `comment-5675223845`에 명령과 결과를 기록했으며 두 이슈 모두 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #8 `command2.c:burn` C `EQUAL`/`find_obj` selector parity `ctx_47a7e10510c8` / `task_e47141e91778` (report `/tmp/g3-burn-equal.md`). direct Inventory-only display/key[0..2] case-insensitive prefix·root 1회 occurrence·OINVIS/PDINVI를 적용하고 cooldown/PHIDDN·보호·RNG/timer·atomic receipt/replay를 보존했다. `burn.go`, `burn_test.go`만 변경했고 worker/coordinator focused world/session/transport race·vet·gofmt·diff-check·scope/hash PASS 후 release 완료. #8 issue comment `comment-5675417355`를 REST 기록했고 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #9 `magic1.c:readscroll` selector parity `ctx_ed2192541449` / `task_a3e9a1d50892` (report `/tmp/g3-readscroll-equal.md`). direct Inventory 우선, display/key[0..2] prefix·OINVIS/PDINVI·root 1회 계산과 독립 Ready occurrence reset을 적용하고 기존 spell/admission/PHIDDN/atomic/replay 경계를 보존했다. 실제 diff는 `read_scroll.go`, `read_scroll_test.go` 두 파일이며 worker/coordinator focused race·gofmt·diff-check·protected hash 및 정착 후 world vet PASS; worker payload의 test-only 경로 표기는 coordinator가 실제 diff로 보정했다. #9 issue comment `comment-5675417354`를 REST 기록했고 OPEN 유지.
- 2026-09-15 정착한 Luna max slice: #9 `magic3.c:turn` selector parity `ctx_527376c45179` / `task_5bd56d0fd620` (report `/tmp/g3-turn-equal.md`). direct room NPC root의 display/Keys[0..2] case-insensitive prefix·stored order·root 1회 occurrence와 MINVIS/PDINVI·PDMINV/validation 경계를 보존했다. 실제 diff는 `turn.go`, `turn_test.go` 두 파일이며 worker/coordinator focused Turn 및 world/session/transport race·vet·gofmt·diff-check·protected hash PASS 후 release 완료; 초기 burn-test 공유 compile obstruction은 burn owner가 해결했다. #9 issue comment `comment-5675417371`를 REST 기록했고 OPEN 유지.
- 현재 `gpt-5.6-luna` / `max` active worker는 0개다. 정착한 #10 `command5.c:who` mode parity `ctx_2c795061f5c8` / `task_640072d3980c` (report `/tmp/g10-who-mode.md`, social status/session 4개 파일 소유)는 worker_done·coordinator 독립 race/vet/gofmt/diff 검증·release 완료했다. #10 issue comment `comment-5676165923`를 REST로 기록했고 이슈는 OPEN 유지한다. #8 공통 Inventory EQUAL selector `ctx_d394dd99dc2d` / `task_caa0f6fd2878` (report `/tmp/g3-inventory-equal.md`)와 #12 ARM64 report-only `ctx_26ac84424e59` / `task_dbbc06845e53` (report `/tmp/g5-arm64-smoke.md`)도 worker_done·coordinator 검증·release 완료했다. Project #8/#9/#10/#12는 `In Progress`, 이슈는 OPEN이다. GraphQL reset 후 #8/#9/#10/#12 Evidence를 갱신하고 item-list로 상태·내용을 재확인했다; #11 Evidence는 기존 최신 근거가 이미 반영되어 있었다. HEAD는 `4e6055e`, `src/frp.new` SHA-256 `d6c16b0ec01a1a4e07391c838b8ee63819e7353b0ece86d293d92373c30906eb`·Git blob `c59adca8b8c37cdde91d5ae71300eea1b5e57c21`·dirty 상태를 유지한다.
- #11 disposable PostgreSQL `pg_dump/pg_restore` `ctx_a05309d87e17` / `task_9b6ae6d3e12b`와 #9 `command8.c:circle` `ctx_e30e083c50ea` / `task_e3b340f9e964`는 worker_done·coordinator 검증·release 완료로 정착했고, 관련 issue comment 중복 없이 유지한다.

- 2026-09-15 정착한 Luna max slice: #10 family-status direct handler acceptance `ctx_ee7b16d4f094` / `task_29483576b1b3` (report `/tmp/g10-family-status-handler.md`, worker_done `msg_43583e592e43`)는 `server/internal/session/family_status_command_test.go` 한 파일만 추가했다. `패거리누구`·`패거리원`·`모든패거리`의 `ParseFamilyLine`/중앙 `ParseCommand`, deterministic who/member/list receipt·replay·불변 snapshot·pre-receipt fail-closed를 검증했고 worker/coordinator race 2회·vet·gofmt·diff-check PASS 후 release했다. #10 issue comment `comment-5676471061`와 Project Evidence를 갱신했으며, chat/board/mail/group/family/admin 전체·PG/E2E/browser/mobile/deploy는 남아 이슈는 OPEN 유지한다.
...
## 이전 오케스트레이터 핸드오프 — 2026-09-14 (기록 보존, 현재 지시 아님)

이 섹션은 이전 재개용 기록이다. 현재 권위는 위의 「RESUMED」 블록과 `docs/porting-research/orchestrator-resume.md`의 최신 블록이다.
...
난수→spell-fail 순서, INT·PRMAGI 보정·LT_FEARS를 receipt에 고정하고, 봉합구는 MP 12·
SUB_DM 이상 gate·고정 3,600초·PRMAGI 절반·LT_SILNC를 고정한다. 두 주문 모두 성공 시
PINVIS를 해제하고 상태·global LT_SPELL·MP를 원자 반영하며, 실패·replay에서는 난수와
room fan-out을 중복하지 않는다.
