...
`2b82e32` (`feat: add durable player settings command slice`)이고, 이후 검증 문서
커밋 `b021dd3`이 추가되어 있다. 이전 기능 기준은 `8535f64` (`docs: record peek checkpoint`)였다.
인프라의 검토된 소스 고정 커밋은 `1434134e`이며 루트의 현재
...
`숨겨`/`숨어`, `엿봐 <대상>`, 제한된 원작 감정표현 alias, 자유 문장 `표현`, 같은 방의
