...
psql -X -h 127.0.0.1 -U postgres -d replay_restore_check -v ON_ERROR_STOP=1 -At \
  -c "select encode(payload,'hex') $query" > "$work/payload.hex"
digest="$(psql -X -h 127.0.0.1 -U postgres -d replay_restore_check -v ON_ERROR_STOP=1 -At -c "select snapshot_sha256 $query")"
...
echo 'GREEN restored DB snapshot passed C clone roundtrip and digest-bound Rust replay'
