...
`InspectBankSnapshotReview(JSON)`과 terminal signup/reconnect 계약용
`web/lib/terminal-play-smoke.ts`도 추가했다. legacy bank 파일 parser·계정 대조·라이브
bank 명령/잔액 parity·CLI wiring·운영 복구는 여전히 P0 후속 항목이다.
...
| bank files / gold | `bank.c`, `bank_store.c`, `bank_snapshot_v1*`, `bank_money_*` | bank item graph and gold transaction with idempotent retry/conflict semantics | bank unit and PG adapter tests; Go kind-8 codec plus `ImportBankSnapshot` atomic success/replay/conflict/rollback; live capture explicitly gap-documented | **부분 구현**; artifact codec와 operator-owned PG import/evidence는 완료, live capture·runtime transfer·복구는 미구현 |
