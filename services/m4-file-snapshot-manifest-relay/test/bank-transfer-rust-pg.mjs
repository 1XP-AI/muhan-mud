  ...
  if(qualified) {
    ...
    await db.query(`grant execute on function ${playerRecoverySignature} to mud_writer`)
    {
      ...
      try {
        ...
        assert.equal(peerState.revision,'1');assert.deepEqual(peerState.bank_payload,bank)
        assert.deepEqual(peerState.player_payload,playerGold(peerState.player_payload,201n))
        ...
        assert.deepEqual(equipmentPlan.stdout.subarray(8,8+equipmentPlayerLength),playerGold(peerState.player_payload,200n))
