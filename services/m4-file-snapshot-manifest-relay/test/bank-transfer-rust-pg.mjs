  ...
  if(qualified) {
    ...
    await db.query(`grant execute on function ${playerRecoverySignature} to mud_writer`)
    {
      ...
      try {
        ...
        assert.equal(peerState.revision,'2');assert.deepEqual(peerState.bank_payload,bank)
        assert.deepEqual(peerState.player_payload,playerGold(peerState.player_payload,202n))
        assert.notDeepEqual(peerState.player_payload,playerGold(peerInitial,202n),'equipped/inventory objects must persist')
        const peerPending=await readPlayerPending(pending,'c9300000-0000-0000-0000-000000000001')
        assert.equal(peerPending.args[4],peerId);assert.deepEqual(peerPending.payload,playerGold(peerState.player_payload,201n))
        const peerNext=await readPlayerPending(pending,'c9300000-0000-0000-0000-000000000002')
        assert.equal(peerNext.args[6],'1');assert.equal(peerNext.args[7],sha(peerPending.payload).toString('hex'))
        assert.deepEqual(peerNext.payload,peerState.player_payload)
        await verifyPlayerResolved(pending,peerPending.args,peerPending.payload)
        await verifyPlayerResolved(pending,peerNext.args,peerNext.payload)
        assert.equal((await db.query('select count(*)::int n from private.game_character_player_save_intents where character_id=$1',[peerId])).rows[0].n,2,'old request retries plus newer state must create exactly two intents')
        ...
        assert.deepEqual(equipmentPlan.stdout.subarray(8,8+equipmentPlayerLength),playerGold(peerState.player_payload,201n))
