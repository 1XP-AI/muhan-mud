...
`--fulfill-pending-snapshot-eligibility` is a separate, finite command with no receipt or player-file scan. It requires `ONBOARDING_SNAPSHOT_ELIGIBILITY_DATABASE_URL` authenticated directly as `onboarding_snapshot_eligibility_login` and `MUD_WRITER_DATABASE_URL` authenticated as `mud_writer_login`; it lists at most `ONBOARDING_SNAPSHOT_ELIGIBILITY_LIMIT` rows (default `100`, maximum `1000`) through the private list-only RPC, validates every returned actor/correlation/character/mode/command tuple as one bounded batch, and invokes only `private.fulfill_game_character_onboarding_snapshot_eligibility(character_id, command_id)` under `SET ROLE mud_writer`. The list connection never sets another role and has database CONNECT, read-only bounded session defaults, private-schema usage, and execute access to only the list RPC; `FULFILLED`, `EXACT_RETRY`, `ALREADY_FULFILLED`, and `NOT_ELIGIBLE` are terminal counted results. Its aggregate JSON includes `alreadyFulfilled`; that terminal result never consumes a retry or makes the command fail. Malformed list rows fail closed before any fulfillment call, and only transient adapter failures retry the unchanged pair up to `ONBOARDING_RECONCILER_RPC_ATTEMPTS`.
