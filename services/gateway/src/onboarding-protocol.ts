...
type HexNameControl = { type: 'RESERVE' | 'VERIFIED'; nameHex: string; fileSha256?: string }
...
export class OnboardingControlLineParser {
  ...
  push(chunk: string | Uint8Array): OnboardingControl[] {
    ...
    if (bytes.length > MAX_CONTROL_LINE_BYTES * 8 || bytes.some((byte) => byte === 0 || byte > 0x7f)) fail()
...
export type OnboardingProtocolState = 'RESERVE' | 'RESERVED' | 'VERIFIED' | 'CLAIMED' | 'SAVED' | 'COMMIT' | 'ERR' | 'ABORT'
const LEGAL_TRANSITIONS: Readonly<Record<OnboardingProtocolState, readonly OnboardingProtocolState[]>> = {
  RESERVE: ['RESERVED', 'ERR'], RESERVED: ['SAVED', 'ERR'], VERIFIED: ['CLAIMED', 'ERR'],
...
const SAFE_LOG_VALUES: Readonly<Record<string, ReadonlySet<string>>> = {
  ...
  state: new Set(['RESERVE', 'RESERVED', 'VERIFIED', 'CLAIMED', 'SAVED', 'COMMIT', 'ERR', 'ABORT']),
  controlType: new Set(['OK', 'RESERVE', 'RESERVED', 'VERIFIED', 'CLAIMED', 'SAVED', 'COMMIT', 'ERR', 'ABORT']),
