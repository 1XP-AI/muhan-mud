...
ws.on('message', (data: RawData, isBinary: boolean) => {
  ...
  if (!isBinary) {
    ...
    if (type === 'error' || type === 'closed') return finish(new Error('gateway rejected real C relay'))
    if (type === 'ready' && !ready) {
      ready = true
  ...
  output.push(bytes)
  if (Buffer.concat(output).includes(healthMarker)) finish()
})

ws.on('error', () => finish(new Error('websocket transport failed')))
ws.on('close', () => {
  if (!finished) finish(new Error('websocket closed before real C response'))
})
