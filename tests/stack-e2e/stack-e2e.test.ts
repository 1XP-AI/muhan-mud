...
async function main(): Promise<void> {
  ...
  try {
    ...
    try { await eventually(async () => {
      ...
      const gates = await sql(`select json_build_object('receipt_after_binding',r.acknowledged_at>b.bound_at,'artifact_ack_equal',a.receipt_acknowledged_at=r.acknowledged_at,'manifest_ack_equal',m.receipt_acknowledged_at=r.acknowledged_at,'artifact_route_equal',(a.world_id,a.legacy_name_key,a.storage_format)=(c.world_id,c.legacy_name_key,c.storage_format),'receipt_route_equal',(r.world_id,r.legacy_name_key,r.storage_format)=(c.world_id,c.legacy_name_key,c.storage_format),'source_octets_equal',m.snapshot_octets=a.source_octets,'manifest_format_equal',m.snapshot_format='legacy-file-manifest-v1','payload_valid',private.player_snapshot_v1_payload_valid(a.payload))::text from private.game_character_onboarding_snapshot_command_bindings b join public.game_characters c on c.id=b.character_id join private.game_character_shadow_receipts r using(character_id,command_id) join private.game_character_player_snapshot_v1_artifacts a using(character_id,command_id) join private.game_character_m4_file_snapshot_manifests m using(character_id,command_id) where b.correlation_id='${webProvisionCorrelation}'`).catch(() => 'diagnostic-unavailable')
      process.stderr.write(`stack-e2e: browser-snapshot-gates=${gates}\n`)
      throw error
    }
