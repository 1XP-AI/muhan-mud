...
export async function runWebStackAcceptance({
  ...
  try {
    ...
    const provisionTerminal = provisionPage.getByLabel("캐릭터 온보딩 터미널");
    await submitOnboardingInput(provisionPage, provision.characterName);
    await expect(provisionTerminal).toContainText(/하시겠습니까/);
    await submitOnboardingInput(provisionPage, "예");
    await expect(provisionTerminal).toContainText(/엔터.*누르십시요/);
    await submitOnboardingInput(provisionPage, "");
    await expect(provisionTerminal).toContainText(/당신은 남자입니까/);
    await submitOnboardingInput(provisionPage, provision.gender);
    await expect(provisionTerminal).toContainText(/직업을 고르세요/);
    await submitOnboardingInput(provisionPage, provision.characterClass);
    await expect(provisionTerminal).toContainText(/능력:/);
    await submitOnboardingInput(provisionPage, provision.stats);
    await expect(provisionTerminal).toContainText(/익숙한 무기를/);
    await submitOnboardingInput(provisionPage, provision.weapon);
    await expect(provisionTerminal).toContainText(/성향을 고르십시요/);
    await submitOnboardingInput(provisionPage, provision.alignment);
    await expect(provisionTerminal).toContainText(/종족/);
