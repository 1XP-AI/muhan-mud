...
{char root[PATH_MAX];character_save_journal_v2_writer_context ctx;mock m;evidence_snapshot a,b,c,A,B,C;int failed=0;if(setup_ack(root,&ctx))return 1;memset(&m,0,sizeof(m));m.result=CHARACTER_SAVE_JOURNAL_V2_RECEIPT_ACKED;character_save_journal_v2_ack_faults_for_test(0,0,0,1,0,0,0);if(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)!=CHARACTER_SAVE_JOURNAL_V2_ACK_DB_ACKED_LOCAL_INCOMPLETE)return 1;if(unlink_leaf(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked")||leaf(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked","conflict",8)||snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked",&a)||snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&b)||a.st.st_nlink!=1||b.st.st_nlink!=1||(a.st.st_dev==b.st.st_dev&&a.st.st_ino==b.st.st_ino))return 1;m.calls=0;failed+=bad(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)==CHARACTER_SAVE_JOURNAL_V2_ACK_DB_ACKED_LOCAL_INCOMPLETE&&m.calls==1&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked",&A)==0&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&B)==0&&same_snapshot(&a,&A)&&same_snapshot(&b,&B),"conflicting different-inode evidence is unchanged after exact callback replay");if(character_save_journal_v2_writer_close(&ctx)||down(root))return failed+1;if(setup_ack(root,&ctx))return failed+1;memset(&m,0,sizeof(m));m.result=CHARACTER_SAVE_JOURNAL_V2_RECEIPT_ACKED;character_save_journal_v2_ack_faults_for_test(0,0,0,1,0,0,0);if(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)!=CHARACTER_SAVE_JOURNAL_V2_ACK_DB_ACKED_LOCAL_INCOMPLETE)return failed+1;{char t[PATH_MAX],q[PATH_MAX];if(p(t,sizeof(t),root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked")||p(q,sizeof(q),root,"character-save-journal/ack-alias")||link(t,q)||snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked",&a)||snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&b)||snapshot(root,"character-save-journal/ack-alias",&c))return failed+1;}m.calls=0;failed+=bad(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)==CHARACTER_SAVE_JOURNAL_V2_ACK_DB_ACKED_LOCAL_INCOMPLETE&&m.calls==1&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked",&A)==0&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&B)==0&&snapshot(root,"character-save-journal/ack-alias",&C)==0&&same_snapshot(&a,&A)&&same_snapshot(&b,&B)&&same_snapshot(&c,&C),"nlink3 alias evidence is unchanged after exact callback replay");if(character_save_journal_v2_writer_close(&ctx)||down(root))return failed+1;if(setup_ack(root,&ctx)||leaf(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp","partial",7)||snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&a))return failed+1;memset(&m,0,sizeof(m));m.result=CHARACTER_SAVE_JOURNAL_V2_RECEIPT_ACKED;failed+=bad(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)==CHARACTER_SAVE_JOURNAL_V2_ACK_DB_ACKED_LOCAL_INCOMPLETE&&m.calls==1&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&A)==0&&same_snapshot(&a,&A),"malformed temp evidence is unchanged after exact callback replay");if(character_save_journal_v2_writer_close(&ctx)||down(root))return failed+1;return failed;}
static int local_marker_deferred_and_freeze(void)
{char root[PATH_MAX];character_save_journal_v2_writer_context ctx;mock m;evidence_snapshot before,after;int failed=0;if(setup_ack(root,&ctx)||leaf(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp","partial",7)||snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&before))return 1;memset(&m,0,sizeof(m));m.result=CHARACTER_SAVE_JOURNAL_V2_RECEIPT_DEFERRED;failed+=bad(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)==CHARACTER_SAVE_JOURNAL_V2_ACK_DEFERRED&&m.calls==1&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&after)==0&&same_snapshot(&before,&after),"malformed local marker preserves deferred callback result and evidence");m.calls=0;m.result=CHARACTER_SAVE_JOURNAL_V2_RECEIPT_INVALID_FREEZE;failed+=bad(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)==CHARACTER_SAVE_JOURNAL_V2_ACK_INVALID_FREEZE&&m.calls==1&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&after)==0&&same_snapshot(&before,&after),"malformed local marker preserves invalid-freeze callback result and evidence");m.calls=0;m.result=CHARACTER_SAVE_JOURNAL_V2_RECEIPT_REJECTED_FREEZE;failed+=bad(character_save_journal_v2_ack(&ctx,CMD,receipt,&m)==CHARACTER_SAVE_JOURNAL_V2_ACK_REJECTED_FREEZE&&m.calls==1&&snapshot(root,"character-save-journal/40000000-0000-0000-0000-000000000001.acked.tmp",&after)==0&&same_snapshot(&before,&after),"malformed local marker preserves rejected-freeze callback result and evidence");if(character_save_journal_v2_writer_close(&ctx)||down(root))return failed+1;return failed;}
...
int main(void)
{
    ...
    failed+=local_marker_deferred_and_freeze();
