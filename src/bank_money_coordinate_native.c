    ...
    const char *const args[11],int timeout_ms,bank_money_coordinate_result *out)
{
    ...
    const char *plan_args[4]; char expected[32]; int i,status=BANK_MONEY_COMMIT_NOT_SENT;
    ...
    if(bank_money_plan_native(planner,plan_args,input.frame,input.frame_length,timeout_ms,&planned,&length)) goto done;
    status=bank_money_commit_prepared_native(connection,node,script,root,args,planned,length,timeout_ms,&revision);
