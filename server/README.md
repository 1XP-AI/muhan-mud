...
폐기한다. `post/ISSUE` raw 파일은 UTF-8/EUC-KR·CRLF·크기·trailing/truncation을 검증하는
`VoteCatalog` parser로 읽고 `-vote-issue-file`/`MUD_VOTE_ISSUE_FILE`로 world 서버에만
주입한다. world/session/transport 전체 race·vet와 통합 gate가 통과했다. 실제 운영
Supabase 대규모 이관·운영 파일 승인/보관, 브라우저/WSS/Ingress·testnet 및 전체 명령
parity는 미완료다.
